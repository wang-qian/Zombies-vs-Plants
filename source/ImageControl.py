...
import time
class ImageControl(pygame.sprite.Sprite):

    def __init__(self,imageFile,number):
        self.imageFile = imageFile 
        self.number = number

    def load_image(self):
        try:
            surface = pygame.image.load(self.imageFile).convert_alpha()
        except pygame.error:
            raise SystemExit('Could not load image "%s" %s'%(self.imageFile, pygame.get_error()))
        width = surface.get_width()
        height = surface.get_height()
        return [surface.subsurface(Rect((0, i*height/self.number), (width, height/self.number))) for i in xrange(self.number)]
